...
app.get("/filter", (req, res) => {
    ...
    const filteredJokes = jokes.filter((joke) => joke.jokeType === typeFilter);
    res.json(filteredJokes)
});

//4. POST a new joke
app.post("/jokes", (req, res) => {
    const maxIndex = Math.max(...jokes.map(joke => joke.id));
    const newJoke = {
        id: maxIndex + 1,
...
app.patch("/jokes/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const jokeToPatch = jokes.find((joke) => joke.id === id);
    const replacementJoke = {
        id: id,
        jokeText: req.body.text || jokeToPatch.jokeText,
        jokeType: req.body.type || jokeToPatch.jokeType,
    };
